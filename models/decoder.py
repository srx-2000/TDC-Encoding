...
import torch
from my_model.models.neural import MultiHeadedAttention, PositionwiseFeedForward, DecoderState
import numpy as np
import math
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory_bank, state, memory_lengths=None,
        ...
        # 这里的state的作用主要是为了后面如果有人需要接入这个transformer解码器，然后再在这个解码器后面继续做什么处理才需要用到，
        # 这里我直接将transformer的这个解码器作为了最后的输出所以不需要使用state这个值了，直接使用output即可
        return output, state
...
#     def __init__(self, config, dropout_p=0.5):
#         super(Decoder, self).__init__()
#         self.config = config
#         self.embedding = nn.Embedding(self.config.vocab_size, self.config.hidden_size)
#         self.transformer_decoder = TransformerDecoder(num_layers=6, d_model=self.config.hidden_size, heads=8, d_ff=2048,
#                                                       dropout=dropout_p,
...
if __name__ == '__main__':
    # encoder = Encoder()
    # decoder = Decoder(encoder.config)
    # print(decoder)
    pass
